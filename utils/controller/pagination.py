...
class CustomPageNumberPaginationExtra(PaginationBase):
    class Input(Schema):
    ...
    def create_input(self) -> Type[Input]:
        class DynamicInput(CustomPageNumberPaginationExtra.Input):
            page: int = Field(1, gt=0)
    ...
    def paginate_queryset(
        ...
        **params: DictStrAny,
        ...
        try:
            url = self.change_url(request=request)
    ...
    def change_url(self, request: Optional[HttpRequest] = None):
        url = request.build_absolute_uri()

        # This regex will match everything from the start up to the first '/'
        return re.sub(r'^(https?://[^/]+)', django_settings.BACKEND_URL, url)
